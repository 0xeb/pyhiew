...
HEM_BYTE hiewSdkVerMajor, hiewSdkVerMinor, hiewVerMajor, hiewVerMinor;
...
////////////////////////////////////////////////////////////
int HEM_API Hem_EntryPoint_1(HEMCALL_TAG *hemCall)
{
...
#define  MENU_WIDTH 30
  ...
  HEM_FNKEYS  fnKeys =
  {
    ...
    "010000000000|      Active      Pasive                                                ",   // main Fn
    "",   // no Alt-Fn
    "",   // no Ctrl-Fn
    "000000000100|                                                      Exit              " // ShiftFn
  };

  HEM_BYTE info[INFO_LINES ][INFO_LINE_MAXLEN];
  HEM_BYTE *pInfo[INFO_LINES];
  HEM_BYTE *menu[] =
  {
    "Hiew version",
    "HEMCALL_TAG view",
  ...
  HEM_BYTE name[LINEBYE_MAXLEN];
  HEM_BYTE lineBye[LINEBYE_MAXLEN * 2];
  int i, item, rc;
  HEM_UINT pressedFnKey;
  HIEWGATE_GETDATA hiewData;
  HIEWGATE_GETHEM2HEMGATE hemGate;
...

  {
    ...
    {
      if( pressedFnKey == HEM_FNKEY_F2 )
        HiewGate_Message( "", "F2 is active and pressed" );
      if( pressedFnKey == HEM_FNKEY_F4 )
        HiewGate_Message( "", "F4 is NEVER press" );
      if( pressedFnKey == HEM_FNKEY_SHIFTF10 )
        return( HEM_OK );
    }
    else if( item == 1 )
    {  
    ...
    {  
      // "Message wait test"; SDK version 0.30
...
#define  MENU_WIDTH 30
  ...
  while (1)
  {
    ...
    if (pressedFnKey != 0)
    {
      if (pressedFnKey == HEM_FNKEY_F2)
        HiewGate_Message("", "F2 is active and pressed" );
      else if (pressedFnKey == HEM_FNKEY_F4)
        HiewGate_Message("", "F4 is NEVER press" );
      else if (pressedFnKey == HEM_FNKEY_SHIFTF10)
        return HEM_OK;
    }
    else if (item == 1)
    {
